  ...
  return (
    <section className='flex flex-col items-center gap-8'>
      ...
      <ul className='w-full max-w-[500px]'>
        {reviews.map(({ reviewerName, comment, rating, date }, index) => (
          <motion.li
          ...
          >
            <div className='flex flex-col gap-2 p-4 w-full border border-border-primary rounded-md overflow-hidden'>
              <div className='flex justify-between items-center gap-2'>
                <span className='text-opposite-theme-primary'>{reviewerName}</span>
                <ProductRatingBadge className='relative' rating={rating} />
              </div>
              <p>{comment}</p>
